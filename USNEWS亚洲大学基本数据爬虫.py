...
retry_strategy = Retry(
    total=30,  # 重试次数
    backoff_factor=2,  # 重试间隔时间，指数退避策略
...
def random_delay(min_seconds=1, max_seconds=30):
...
def fetch_data(start_page, end_page):
    for page in range(start_page, end_page):
        url = f'https://www.usnews.com/education/best-global-universities/search?format=json&region=asia&page={page}'
        headers = {'User-Agent': random.choice(user_agent_list)}

        for attempt in range(30):  # 最多尝试30次
            try:
                ...
                logging.info(f"Using User-Agent: {headers['User-Agent']}")
                 ...
                 #   continue
                response = requests.get(
                    url='https://www.usnews.com/education/best-global-universities/search?format=json&region=asia&page='+str(page),
                    #headers=headers,
                    params={'param': '1'}, 
                    headers=headers,
                    #proxies=proxy,
                ...
                if response.status_code == 200:
                    try:
                        ...
                        with open('collegeInfo_aisa.txt', 'a', encoding='utf-8') as fp:
                            for i in li:
