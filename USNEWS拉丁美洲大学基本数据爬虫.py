...
def fetch_data(start_page, end_page):
    for page in range(start_page, end_page):
        url = f'https://www.usnews.com/education/best-global-universities/search?format=json&region=latin-america&page={page}'
        ...
        for attempt in range(30):  # 最多尝试5次
            try:
                ...
                logging.info(f"Using User-Agent: {headers['User-Agent']}")
                 ...
                 #   continue
                response = requests.get(
                    url='https://www.usnews.com/education/best-global-universities/search?format=json&region=latin-america&page='+str(page),
                    #headers=headers,
                    params={'param': '1'}, 
                    headers=headers,
                    #proxies=proxy,
                ...
                if response.status_code == 200:
                    try:
                        ...
                        with open('collegeInfo_latin-america.txt', 'a', encoding='utf-8') as fp:
                            for i in li:
